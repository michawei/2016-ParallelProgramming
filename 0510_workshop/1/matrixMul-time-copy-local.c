...
int main(int argc, char *argv[])
{
  ...
  //assert(status == CL_SUCCESS);
  /* kernelsource */
  FILE *kernelfp = fopen("mul-local-kernel.cl", "r");
  ...
  /* setshape */
  size_t globalThreads[] = {(size_t)a, (size_t)b, (size_t)c};
  size_t localThreads[] = {d, e, f};
  cl_event event;
  
  cl_ulong number;
  clGetDeviceInfo(GPU[0], CL_DEVICE_MAX_WORK_GROUP_SIZE , sizeof(cl_ulong), &number , NULL);
  printf("CL_DEVICE_MAX_WORK_GROUP_SIZE = %ld\n", number);

  cl_ulong dim[3];
  clGetDeviceInfo(GPU[0], CL_DEVICE_MAX_WORK_ITEM_SIZES , 3 * sizeof(cl_ulong), &dim , NULL);
  printf("CL_DEVICE_MAX_WORK_ITEM_SIZES = %ld %ld %ld\n", dim[0], dim[1], dim[2]);

  if ( a % d != 0 || b % e != 0 || c % f != 0 )
    printf(" INVALID_DIMENSIONS");
  if ( d > dim[0] || e > dim[1] || f > dim[2] )
    printf(" INVALID_DIMENSIONS");
  if ( d * e * f > (int)number )
    printf(" INVALID_WORK_GROUP_SIZE\n");
